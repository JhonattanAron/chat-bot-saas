...
"[project]/store/AutomatizedTaskStore.ts [app-ssr] (ecmascript)": ((__turbopack_context__) => {
"use strict";

var { g: global, __dirname } = __turbopack_context__;
{
__turbopack_context__.s({
    "useAutomatedTasksStore": (()=>useAutomatedTasksStore)
});
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zustand$2f$esm$2f$react$2e$mjs__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/zustand/esm/react.mjs [app-ssr] (ecmascript)");
;
const NEST_API_URL = process.env.NEST_API_URL || "http://localhost:8080";
const useAutomatedTasksStore = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zustand$2f$esm$2f$react$2e$mjs__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["create"])((set)=>({
        tasks: [],
        currentTask: null,
        logs: [],
        loading: false,
        ...
        getTask: async (id, user_id)=>{
            ...
            try {
                const res = await fetch(`${NEST_API_URL}/automated-tasks/${id}?user_id=${user_id}`);
                const json = await res.json();
                if (json.success) {
                    set({
        ...
        toggleTaskStatus: async (id, user_id)=>{
            try {
                const res = await fetch(`${NEST_API_URL}/automated-tasks/${id}/toggle-status?user_id=${user_id}`, {
                    method: "POST"
                });
                const json = await res.json();
                if (json.success) {
        ...
        executeTask: async (id, user_id, context)=>{
            try {
                const res = await fetch(`${NEST_API_URL}/automated-tasks/${id}/execute?user_id=${user_id}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"
                    },
        ...
        getTaskLogs: async (id, user_id, limit = 50)=>{
            try {
                const res = await fetch(`${NEST_API_URL}/automated-tasks/${id}/logs?user_id=${user_id}&limit=${limit}`);
                const json = await res.json();
                if (json.success) {
                    set({
                        logs: json.logs
        ...
        handleWebhook: async (taskId, payload)=>{
            try {
                const res = await fetch(`${NEST_API_URL}/automated-tasks/webhook/${taskId}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"
                    },
...
//# sourceMappingURL=store_d6cae1b9._.js.map