...
export default function CreateBotPage() {
  ...
  const { createAssistant, assistants } = useChatAssistantStore();
  const { data: session } = useSession();
  const { userPlan, fetchUserPlan, checkLimits } = usePlansStore();
  ...
  }, [session?.binding_id, fetchUserPlan]);
  ...
  const maxBots = userPlan?.plan.limits.chatbots || 3;
  const planName = userPlan?.plan.name || "Básico";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (currentBotCount >= maxBots) {
      toast({
        ...
        description:
          language === "en"
            ? `You've reached the maximum number of bots for your ${planName} plan. Please upgrade to create more bots.`
            : `Has alcanzado el número máximo de bots para tu plan ${planName}. Por favor actualiza tu plan para crear más bots.`,
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="flex flex-col gap-4 p-2 sm:p-4 md:gap-8 md:p-8">
        ...
        <PlanLimitWarning
          ...
          onUpgrade={() => {
            toast({
              title: "Próximamente",
              description:
                "La funcionalidad de actualización de planes estará disponible pronto.",
            });
          }}
        />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <TokenCounter count={welcomeMessageTokens + descriptionTokens} />
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant={currentBotCount >= maxBots ? "destructive" : "default"}
              >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Button
            type="submit"
            disabled={isLoading || currentBotCount >= maxBots}
            className="w-full"
          >
            {isLoading ? (
              <>
